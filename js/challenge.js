...
return setInterval(function(){var a=document.getElementById("counter"),b=parseInt(a.innerText);
    a.innerText=b+1},1e3)},interval=timer(),
        ...
        d.innerHTML=b+ "has been liked <span> +(e+1)+" <'/span> times'}
            else (d=document.createElement("li")).setAttribute("data-num",b),d.innerHTML=b+ " has been liked <span> 1, </span>", 
            (timeconst ('newLocal')) = {
                c.appendChild(d);
            }
                ...
                [].concat(_toConsumableArray(document.getElementsByTagName
                ("button"))).forEach(function(a){
                    "pause"!==a.id&&('a.disabled=!playing')})),commentForm.addEventListener("submit", function(a){a.preventDefault();
